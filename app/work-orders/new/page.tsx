...
export default function NewWorkOrderPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      setEstimateId(new URLSearchParams(window.location.search).get('estimateId'));
    }
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error, data: workOrder } = await supabase.from('work_orders').insert({
      title,
      description,
      status: 'new',
    ...
    if (error || !workOrder) {
      ...
      setMessage(err instanceof Error ? err.message : 'Unable to save work order.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="container">
      <AppNav />
      <h1 style={{ fontSize: '1.5rem', fontWeight: 600, marginBottom: '1rem' }}>New Work Order</h1>
